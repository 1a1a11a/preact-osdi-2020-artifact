...
from change_points import Detector
import constants
...
class Beatlex(Detector):
    ...
    def detect_infant_mortality_end(self, data):
        # self.eng.cd(r'/Users/saukad/devel/reliability_heterogeneity/change_points/implementations/beatlex', nargout=0)
        # afr = []
        for d in data['cum_afr']:
            # afr.append(float(d))
            self._afr.append(float(d))
        starts, errors = self.eng.findchangepts(matlab.double(self._afr), 'MinDistance', 30, 'MaxNumChanges', 2,
                                                'Statistic', 'rms', nargout=3)
        print(starts)
