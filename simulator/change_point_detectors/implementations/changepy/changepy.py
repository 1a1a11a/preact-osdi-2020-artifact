...
import constants
import numpy as np
from change_point_detectors.implementations.changepy.pelt import pelt
from change_point_detectors.implementations.changepy.costs import normal_meanvar


class ChangePy(Detector):
    def __init__(self):
    ...
    def detect_infant_mortality_end(self, data):
        self._afr = np.append(self._afr, data)

        if len(self._afr) < (constants.AGE_EXEMPT_FROM_STABLE_STATE - constants.HYSTERESIS_SLIDING_WINDOW):
            return -1

        print(len(self._afr))
        print(pelt(normal_meanvar(self._afr.tolist()), len(self._afr)))
        exit(0)
